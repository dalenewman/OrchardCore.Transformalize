   ...
   public class Migrations : DataMigration {
      ...
      public async Task<int> UpdateFrom9Async() {
         ...
         await _contentDefinitionManager.AlterPartDefinitionAsync("TransformalizeReportPart", part => part
            .WithField("Chart", field => field
               .OfType(nameof(BooleanField))
               .WithDisplayName("Chart")
               .WithPosition("37")
               .WithSettings(new BooleanFieldSettings {
                  DefaultValue = false,
                  Hint = "Display a field's data as a chart."
               })
            )
            .WithField("ChartField1", field => field
               .OfType(nameof(TextField))
               .WithDisplayName("Dataset 1")
               .WithPosition("38")
               .WithSettings(new TextFieldSettings {
                  Hint = "Specify a field's name for the first dataset."
               })
            )
            .WithField("ChartField2", field => field
               .OfType(nameof(TextField))
               .WithDisplayName("Dataset 2")
               .WithPosition("39")
               .WithSettings(new TextFieldSettings {
                  Hint = "Specify a field's name for the second dataset."
               })
            )
            .WithField("ChartField3", field => field
               .OfType(nameof(TextField))
               .WithDisplayName("Dataset 3")
               .WithPosition("40")
               .WithSettings(new TextFieldSettings {
                  Hint = "Specify a field's name for the third dataset."
               })
            )
            .WithField("ChartType", field => field
               .OfType(nameof(TextField))
               ...
               .WithSettings(new TextFieldPredefinedListEditorSettings {
                  Options =
                  [
                     new ListValueOption { Name = "Doughnut", Value = "doughnut" },
                     new ListValueOption { Name = "Stacked bar", Value = "stacked bar" },
                     new ListValueOption { Name = "Stacked bar/line", Value = "stacked bar line" },
                     new ListValueOption { Name = "Horizontal bar", Value = "horizontal bar" },
                     new ListValueOption { Name = "Multi Series Pie", Value = "multi pie" },
                     new ListValueOption { Name = "Line", Value = "line" },
                     new ListValueOption { Name = "Bar", Value = "bar" },
                     new ListValueOption { Name = "Pie", Value = "pie" },
                  ],
                  Editor = EditorOption.Dropdown,
                  DefaultValue = "doughnut"
               })
            )
            .WithField("ChartTitle", field => field
               .OfType(nameof(TextField))
               .WithDisplayName("Chart Title")
               .WithPosition("42")
               .WithSettings(new TextFieldSettings {
                  Hint = "The title of the chart."
               })
            )
            .WithField("ChartDisplayLegend", field => field
               .OfType(nameof(BooleanField))
               .WithDisplayName("Display Legend")
               .WithPosition("43")
               .WithSettings(new BooleanFieldSettings {
                  DefaultValue = true,
                  Hint = "Enable to display the legend."
               })
            )
            .WithField("ChartShowPercentage", field => field
               .OfType(nameof(BooleanField))
               .WithDisplayName("Show Percentage")
               .WithPosition("44")
               .WithSettings(new BooleanFieldSettings {
                  Hint = "Show percentage values instead of raw values on chart hover."
               })
            )
            .WithField("ChartColorPaletteMap", field => field
               .OfType(nameof(TextField))
               .WithDisplayName("Color Palette Map")
               .WithPosition("45")
               .WithSettings(new TextFieldSettings {
                  Hint = "Optional map for color palette. Leave empty to use default colors."
               })
            )
            .WithField("ChartTitleLink", field => field
               .OfType(nameof(LinkField))
               .WithDisplayName("Title Link")
               .WithPosition("46")
               .WithSettings(new LinkFieldSettings {
                  Hint = "Optionally specify a URL to link the chart title to. If left empty the title will link to the report itself.",
                  LinkTextMode = LinkTextMode.Url
               })
            )
            .WithField("ChartUseRawData", field => field
               .OfType(nameof(BooleanField))
               .WithDisplayName("Use Raw Data")
               .WithPosition("47")
               .WithSettings(new BooleanFieldSettings {
                  Hint = "When enabled, the chart will use the actual values from the specified chart field instead of counting occurrences of distinct values."
               })
            )
            .WithField("ChartRawDataLabelField", field => field
               .OfType(nameof(TextField))
               .WithDisplayName("Raw Data Label Field")
               .WithPosition("48")
               .WithSettings(new TextFieldSettings {
                  Hint = "When 'Use Raw Data' is enabled, specify a field whose values will be used as labels for the data points in the chart."
               })
            )
         );
