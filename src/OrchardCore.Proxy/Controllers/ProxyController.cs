        ...
        private static async Task<EmptyResult> RelayContent(
            ...
            try {
                using (resourceResponse = await client.SendAsync(message)) {
                    ...
                    var responseETag = resourceResponse.Content.Headers.GetValues("ETag").FirstOrDefault();
                    if (responseETag != null) {
                        response.Headers["Cache-Control"] = "private, must-revalidate";
                        response.Headers["ETag"] = responseETag;

                        var requestedETag = message.Headers.GetValues("If-None-Match").FirstOrDefault();
                    ...
                    using (var resourceStream = await resourceResponse.Content.ReadAsStreamAsync()) {
                        resourceStream.CopyTo(response.Body);
                    }
